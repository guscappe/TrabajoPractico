...
CREATE TRIGGER trg_actualiza_horas_trabajadas
	...
	begin
		...
		if ( !(NEW.horas IS NULL) ) then 
			...
			VALUES (OLD.horas,NEW.horas,OLD.mes,CURRENT_USER(),OLD.id_rol,OLD.id_project, CURDATE(), diferencia,'U');
			...
			IF ( select COUNT(id_project) FROM liq_mensual WHERE  id_project = OLD.id_project and mes= OLD.mes GROUP BY id_project ) THEN 
				...
				ELSEIF OLD.id_rol=3 then 
					UPDATE liq_mensual SET hs_Tester=@nue_valor WHERE  id_project =OLD.id_project and mes=OLD.mes;
